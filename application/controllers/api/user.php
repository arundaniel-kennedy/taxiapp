...
from users where utype=1 and is_online = 1 HAVING distance < $limit order by distance asc");
    ...
    public function rides_get() {
        ...
        $res = $this->db->query("select r.*,w.mobile as user_mobile,w.avatar as user_avatar,w1.avatar as driver_avatar,w.name as user_name,w1.mobile as driver_mobile,w1.user_id as driver_id,w1.name as driver_name from rides as r left join users as w on r.user_id=w.user_id left join users as w1 on r.driver_id=w1.user_id where $wh and r.status='$status' order by r.ride_id desc limit 20");
